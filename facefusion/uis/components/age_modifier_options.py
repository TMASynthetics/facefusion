...
AGE_MODIFIER_INFER_SOURCE_AGE : Optional[gradio.Button] = None
...
def render() -> None:
	...
	global AGE_MODIFIER_INFER_SOURCE_AGE
	...
	AGE_MODIFIER_INFER_SOURCE_AGE = gradio.Button(
		value = "Source Age Inference",
		size = "sm",
	...
	AGE_MODIFIER_STRIDE_SLIDER = gradio.Slider(
		...
		visible = has_age_modifier and has_fran and is_advanced_user,
		interactive=True
	)
	...
	register_ui_component('age_modifier_infer_source_age', AGE_MODIFIER_INFER_SOURCE_AGE)
...
def listen() -> None:
	...
	AGE_MODIFIER_INFER_SOURCE_AGE.click(get_source_age, inputs=[], outputs = AGE_MODIFIER_SOURCE_AGE_SLIDER)
...
def update_age_modifier_model(age_modifier_model : AgeModifierModel) -> Tuple[gradio.Dropdown, gradio.Slider, gradio.Button, gradio.Slider, gradio.Slider, gradio.Slider, gradio.CheckboxGroup]:
...
def get_source_age() -> int:
	age_estimate = state_manager.get_item('current_source_age_estimate')
	return int(age_estimate) if age_estimate is not None else 20
